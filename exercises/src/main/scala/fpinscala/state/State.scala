...
object RNG {
  ...
  def doubleViaMap: Rand[Double] = {
    map(int) { i =>
      Math.abs(i.toDouble) / (Int.MaxValue.toDouble + 2)
    }
  }
  ...
  def sequence[A](fs: List[Rand[A]]): Rand[List[A]] = {
    fs.foldRight(unit(List()): Rand[List[A]]){ (ra, rla)  => map2(ra, rla)(_ :: _) }
  }
  ...
  def nonNegativeLessThan(n: Int): Rand[Int] = {
    map(int){
      i => ((Math.abs(i.toDouble) / (Int.MaxValue.toDouble + 2)) * n).toInt
    }
  }
